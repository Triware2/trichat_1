-- Payment Integration Migration Script - FIXED VERSION
-- Run this to fix existing table conflicts and enum issues
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        -- Add check constraint after adding the column
    ...
    -- Handle status column - check if it's an enum or text
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'subscriptions' AND column_name = 'status' 
               AND data_type = 'USER-DEFINED') THEN
        -- Status is an enum, we need to handle it carefully
        -- First, let's see what values are currently in the enum
        RAISE NOTICE 'Status column is an enum type. Please check existing values before proceeding.';
    ELSE
        -- Status is text, we can add a check constraint
        BEGIN
            -- Try to add the check constraint, but don't fail if it already exists
...
-- Check the current status column type and values
SELECT 
    column_name,
    data_type,
    udt_name
...
BEGIN
    ...
    IF enum_name IS NOT NULL AND enum_name != 'text' THEN
        RAISE NOTICE 'Status column uses enum type: %', enum_name;
        -- You can query the enum values here if needed
    END IF;
END $$;
