-- Fixed System Settings Tables Schema
...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE system_settings ADD CONSTRAINT system_settings_category_check 
            CHECK (category IN ('general', 'notifications', 'security', 'integrations', 'maintenance'));
    END IF;
END $$;

-- Fix value column type if it's JSON
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        SELECT id, category, key, value::TEXT as value, description, created_at, updated_at 
        ...
        -- Copy data back
        INSERT INTO system_settings SELECT * FROM temp_system_settings;
        
        -- Drop temp table
        DROP TABLE temp_system_settings;
    END IF;
END $$;
...
DROP POLICY IF EXISTS "Allow admins to manage system settings" ON system_settings;
DROP POLICY IF EXISTS "Allow admins to manage backups" ON system_backups;
DROP POLICY IF EXISTS "Allow admins to manage maintenance mode" ON maintenance_mode;
DROP POLICY IF EXISTS "Allow admins to view system logs" ON system_logs;
DROP POLICY IF EXISTS "Allow system to insert logs" ON system_logs;
